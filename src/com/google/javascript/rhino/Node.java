  ...
  */
  ...
  private static class StringNode extends Node {
    ...
    public String getString() {
    	/*SUSHI: JBSE does not cope well with symbolic strings.  Workaround: use predefined strings instead*/ if (!Analysis.isResolved(this, "str")) /*if (str == null)*/ {
        	Analysis.assume(strIndex >=0 && strIndex <= RenameLabels.fake_names_index++);
        	str = RenameLabels.fake_names[strIndex];
    	}
...
//SUSHI: TRIGGERS FOR HEX INVARIANTS: BEGIN
//Triggers
  ...
  @SuppressWarnings("unused")
  private static  void _gotLastExpand(Node node) {
	  if(!Analysis.isResolved(node, "first")) return;
	  Node lastChild = node.first;
	  for(Node n = node.first; n != null; n = n.next){
		  if(!Analysis.isResolved(n, "next")) return;
	  }
	  Analysis.assume(false);
  }

//SUSHI: TRIGGERS FOR HEX INVARIANTS: END
  ...
  public String getString() throws UnsupportedOperationException {
    if (this.getType() == Token.STRING) {
      ...
      throw new UnsupportedOperationException(/*SUSHI: JBSE does not cope well with concatenation. Workaround*//*this + " is not a string node"*/);
    }
  }
